...
class GeneratePostmanCollection extends Command
{
    ...
    {
        if (! $this->option('name')) {
            $this->input->setOption('name', basename(base_path()));
        }
    ...
    protected function buildEndpoints(): void
    {
        ...
        // Auth etkinse Core folder'a auth ekle
        if ($this->authEnabled) {
            $coreFolder['auth'] = [
                'type' => 'bearer',
                'bearer' => [
            ...
            // Auth kapalıysa sadece Core folder
            $this->collection['item'] = [$coreFolder];
        }
    }
    ...
    protected function routesToPostman(): array
    {
        ...
        foreach (Route::getRoutes() as $route) {
            ...
            if (! str_starts_with($uri, 'api/v1') || str_contains($uri, '{path}')) {
                continue;
            }

            $this->line("Processing route: {$uri}");
            ...
            foreach (array_diff($route->methods(), ['HEAD']) as $method) {
                ...
                if (in_array($firstSeg, array_unique(array_merge($this->excludedRouteFirstSegments, $configuredSegs)), true)) {
                    continue;
                }
        ...
        foreach (File::glob($path.'/*') as $item) {
            ...
            if (File::isDirectory($item)) {
                ...
                if (strtolower($name) === 'relation' || strtolower($name) === 'relations') {
                    ...
                    if (! empty($sub)) {
                        $structure['Relation'] = ['type' => 'relation', 'parent' => $this->parentModule($rel), 'children' => $sub];
                    }
                } else {
                    $sub = $this->scanModels($item, $rel.'/'.$normalized);
                    if (! empty($sub)) {
                        $structure[$normalized] = ['type' => 'directory', 'children' => $sub];
                    }
                }
    ...
    protected function guessRelationSlug(?string $parentModelFqn, string $relationModelFqn): ?string
    {
        ...
        try {
            ...
            foreach ((new \ReflectionClass($parentModelFqn))->getMethods(\ReflectionMethod::IS_PUBLIC) as $method) {
                if ($method->isStatic() || $method->getNumberOfRequiredParameters() > 0 || $method->isConstructor() || $method->isDestructor()) {
                    continue;
                }
                try {
                    $rel = $method->invoke($parent);
                    if ($rel instanceof \Illuminate\Database\Eloquent\Relations\HasMany && get_class($rel->getRelated()) === $relationModelFqn) {
    ...
    protected function toPostmanTree(array $node): array
    {
        ...
        foreach ($node as $name => $info) {
            if ($info['type'] === 'directory' || $info['type'] === 'relation') {
                ...
                if (! empty($children)) {
                    $items[] = ['name' => $name, 'item' => $children, 'description' => $info['type'] === 'relation' ? "Relation relations for {$info['parent']}" : "Operations for {$name} module"];
                }
            } elseif ($info['type'] === 'model') {
                $requests = $this->makeCrudRequests($name, $info['route'], $info['file_path'], ['is_relation' => false]);
                if (! empty($requests)) {
                    $items[] = ['name' => $name, 'item' => $requests, 'description' => "CRUD operations for {$name}"];
                }
            }
        }
        ...
        foreach ($actions as $action => $cfg) {
            ...
            if ($cfg['param']) {
                $req['request']['url']['variable'] = [
                    ['key' => 'id', 'value' => '1', 'description' => "{$modelName} ID"],
                ];
            }
            ...
            if (in_array($cfg['method'], ['POST', 'PUT'], true)) {
                $req['request']['body'] = [
                    'mode' => 'raw',
                    'raw' => json_encode(
                        $this->jsonBodyParams($filePath, $action),
            ...
            // Authorization header sadece auth etkinse VE gerekiyorsa ekle
    ...
    protected function jsonBodyParams(string $filePath, string $action): array
    {
        ...
        try {
            foreach ((new \ReflectionClass($fieldTrait))->getProperties() as $prop) {
                ...
                if ($value === null) {
                    $value = $this->exampleFromDbType($name, $columnsMeta[$name]['type'] ?? null, $form->type ?? null);
                }
                if ($value === null) {
    ...
    protected function stringExampleFor(string $field): string
    {
        ...
        if (Str::startsWith($field, 'is_') || Str::startsWith($field, 'support_') || Str::contains($field, ['_access', '_friendly'])) {
            return 'true';
        }
    ...
    protected function normalizeDbType(?string $type): ?string
    {
        ...
        if (str_starts_with($t, 'tinyint')) {
            return (preg_match('/tinyint\((\d+)\)/', $t, $m) && (int) $m[1] === 1) ? 'tinyint_bool' : 'smallint';
        }
    ...
    {
        $allSegs = array_unique(array_merge(
            $this->excludedRouteFirstSegments,
            array_map('strtolower', (array) config('postman_generator.excluded_route_first_segments', []))
        ));
        ...
        foreach ($items as $it) {
            ...
            if (isset($it['request']['url']['path']) && is_array($it['request']['url']['path']) && in_array(strtolower($it['request']['url']['path'][0] ?? ''), $allSegs, true)) {
                continue;
            }
            $out[] = $it;
        }
    ...
    {
        if ($bytes >= 1048576) {
            return round($bytes / 1048576, 2, PHP_ROUND_HALF_UP).' MB';
        }
        if ($bytes >= 1024) {
            return round($bytes / 1024, 2, PHP_ROUND_HALF_UP).' KB';
        }
    ...
    {
        if (in_array($base, array_merge($this->excludedModelNames, (array) config('postman_generator.excluded_models', [])), true)) {
            return true;
        }
